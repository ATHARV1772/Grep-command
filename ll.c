...
void display(line l){
    //to display the line i.e., every letter in the line
	node *ptr;
	ptr = l.head;
	while(ptr->next){
		printf("%c", ptr->a[0]);
...
int match_pattern(line l, char *pattern){
	...
	while((q->next)  && (string[i] != '\0')){
		if(string[i] == q->a[0]){
				q = q->next; 
                //if one character matches of line and string increment i 
                //and also point to next character of line
				i++;
...
int match_whole_string(line l, char *pattern){
	...
	while((q->next)  && (string[i]!='\0')){
		if(string[i] == q->a[0] ){
            ...
            //directly set i to 0 again so begins search from beginning of new word in line
            if(i != 0){
                i = 0;
            }
            else{
...
int caseinsensitive_whole(line l, char *pattern){ 
    //for matching case insensitive whole word string patterns
    char string[70];
	strcpy(string, pattern);
	line l1;
	init_line(&l1);
