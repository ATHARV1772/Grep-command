...
int openfile(char *filename, int option, char *pattern, int flag, int num, int option1){ 
    //flag 1 for H and 0 for h and num is for -m
        ...
        while((ch = getc(fp)) != EOF ){
            if (ch != '\n' && ch != '\0'){
                ...
                if (option == EXP || option == RECURSION || option == FILENAME){
						...
						if(boolean){
							if(flag){
								if(option == FILENAME && num == 1){ 
                                    //only when num is 1 print file name this is to avoid printing file name again and again
									printf(" %s \n",filename);
									num++;
									continue;
								}
